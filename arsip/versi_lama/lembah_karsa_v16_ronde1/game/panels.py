...
def render_panel_map(game, bx, by, bw, bh):
    """Mini-map: tampilkan scene aktif (atau outdoor jika di indoor).
    v5: support semua scene baru, tile colors disesuaikan."""
    title = game.font_big.render("🗺 PETA (M tutup)", True, C.ui_gold)
    game.screen.blit(title, (bx + 20, by + 15))

    # Pilih scene yang di-display: jika player di indoor, tampilkan outdoor
    active = game.state.scene_name
    scene = SCENES[active]
    if scene.indoor:
        # Tampilkan outdoor sebagai konteks utama
    ...
    # Player marker
    if game.state.scene_name == active:
        pygame.draw.circle(game.screen, (255, 255, 80),
            (int(mx + game.state.player_x * sx),
             int(my + game.state.player_y * sy)), 3)

    # Label nama scene
    label = game.font_small.render(scene.display, True, C.ui_text)
    game.screen.blit(label, (bx + 20, by + 245))
    ...
    legend = [
        ...
        ("🏠 Bangunan", (180, 80, 60)),
        ("🪦 Kuburan", (90, 80, 80)),
        ("🏮 Pasar/Lentera", (200, 180, 60)),
        ("🚪 Gerbang", (200, 180, 80)),
    ]
    for label_txt, col in legend:
        pygame.draw.rect(game.screen, col, (bx + 280, ly, 12, 12))
        t = game.font_small.render(label_txt, True, C.ui_text)
        game.screen.blit(t, (bx + 295, ly - 1))
        ly += 18

    # Lokasi sekarang (indoor info)
    if game.state.scene_name != active:
        info = game.font_small.render(
            f"📍 Sekarang: {SCENES[game.state.scene_name].display}",
            True, C.ui_cyan)
        game.screen.blit(info, (bx + 280, ly + 8))
...
def render_panel_quest(game, bx, by, bw, bh):
    ...
    for i, stage in enumerate(QUEST_STAGES):
        if i < game.state.quest_stage:
            ...
            col = (100, 80, 140); mark = " "
        t = game.font_small.render(
            f"{mark} {stage['t']}: {stage['d']}", True, col)
        game.screen.blit(t, (bx + 20, y))
        y += 18
...
def render_panel_relations(game, bx, by, bw, bh):
    ...
    for section_label, npcs, col in [
        ...
        for npc_id, data in npcs.items():
            ...
            t = game.font_small.render(
                f"  {data['name']}: {heart_str} ({act})", True, C.ui_text)
            game.screen.blit(t, (bx + 30, y))
            y += 16
        y += 4
...
def build_shop_items(state):
    ...
    for crop_id, crop in CROPS.items():
        if szn in crop['seasons'] or state.greenhouse_open:
            cost = crop['cost'] * 3
            items.append((f"{crop['name']} x3 = {cost}G",
                          ('buy_seed', crop_id, cost)))
...
def build_upgrade_items(state):
    ...
    for upg_id, cost, label in upgrades:
        if state.upgrades.get(upg_id, False):
            ...
            items.append((f"{label} - {cost}G",
                          ('buy_upgrade', upg_id, cost)))
    items.append(("❌ Tutup (ESC)", ('close', None, None)))
    return items
...
def render_panel_shop(game, bx, by, bw, bh):
    title = game.font_big.render(
        f"🏪 WARUNG ({game.state.gold}G)", True, C.ui_gold)
    game.screen.blit(title, (bx + 20, by + 15))
    items = build_shop_items(game.state)
    y = by + 60
    for i, (label, _) in enumerate(items):
        if i == game.panel_select:
            pygame.draw.rect(game.screen, (60, 30, 100),
                             (bx + 10, y - 3, bw - 20, 24))
...
def render_panel_upgrade(game, bx, by, bw, bh):
    title = game.font_big.render(
        f"⚒️ BENGKEL ({game.state.gold}G)", True, C.ui_gold)
    game.screen.blit(title, (bx + 20, by + 15))
