...
Menangani semua logika pemain:
  - Smooth movement 8-arah (keyboard + mouse hold)
  - Penggunaan alat (cangkul, siram, tanam, panen, kapak, hadiah)
  - Interaksi dengan tile (pintu, peti, kasur, dll)
  - Bicara dengan NPC (talk_to_npc, show_schedule)
  - Tangkap makhluk halus (jamur lari, kunang)

Diparse keluar dari Game class app.py v4 supaya app.py tetap tipis.
Composition: PlayerController(game) — semua state ada di game.state, akses
balik ke game via self.game.* untuk dialog/notif/transition.
"""
import pygame
import math
import random
from .config import (
    TILE, VIEW_H, PLAYER_SPEED_TILES_PER_SEC, PLAYER_RUN_MULTIPLIER,
    MOUSE_DEAD_ZONE, DIAGONAL_MULT, TOOLS,
    BLOCKING, WALKABLE, TILLABLE,
    G, MB, BD, CH, ST, BS, MR, FP, CL, TB, GR, LN, DT, PP, CT, TR, DR
)
from .scenes import SCENES
from .data import (CROPS, WILD_ITEMS, HUMAN_NPCS, SUPERNATURAL_NPCS,
                   ANIMAL_NPCS, SCHEDULES)
...
class PlayerController:
    def __init__(self, game):
        self.game = game
        # State movement (mengikuti pattern v4)
    ...
    def get_facing_tile(self):
        ...
        f = self.game.state.facing
        if f == 'up': return (x, y - 1)
        if f == 'down': return (x, y + 1)
        if f == 'left': return (x - 1, y)
        return (x + 1, y)

    def can_walk(self, x, y):
        scene = SCENES[self.game.state.scene_name]
        # Defensive cast untuk safety
        x = int(round(x))
        y = int(round(y))
    ...
    # ═══════════════════════════════════════════════════
    #  MOVEMENT — 8-arah dengan keyboard ATAU mouse hold
    # ═══════════════════════════════════════════════════
    def update_movement(self, dt):
        ...
        if mouse_buttons[0]:
            ...
            if my < VIEW_H * TILE:
                world_x = (mx + self.game.cam_x * TILE) / TILE
                world_y = (my + self.game.cam_y * TILE) / TILE
                tdx = world_x - self.game.state.player_x - 0.5
                tdy = world_y - self.game.state.player_y - 0.5
                pixel_dist = math.hypot(tdx * TILE, tdy * TILE)
                if pixel_dist > MOUSE_DEAD_ZONE:
        ...
        new_x = self.game.state.player_x + dx * speed_per_ms * dt
        new_y = self.game.state.player_y + dy * speed_per_ms * dt

        cur_tx = int(round(self.game.state.player_x))
        cur_ty = int(round(self.game.state.player_y))

        # Slide-along-walls: cek X dan Y secara terpisah
        if dx != 0:
            test_tx = int(round(new_x))
            if self.can_walk(test_tx, cur_ty):
                self.game.state.player_x = new_x
        if dy != 0:
            test_ty = int(round(new_y))
            cur_tx_now = int(round(self.game.state.player_x))
            if self.can_walk(cur_tx_now, test_ty):
                self.game.state.player_y = new_y

        self.move_target_x = 1  # truthy untuk indicate moving

        # Cek portal di tile baru
        new_tx = int(round(self.game.state.player_x))
        new_ty = int(round(self.game.state.player_y))
    ...
    def use_hoe(self, x, y, tile):
        ...
        self.game.state.soil[key] = {
            'tilled': True, 'watered': False, 'crop': None, 'age': 0
        }
    ...
    def use_watering(self, x, y):
        ...
        for cx, cy in cells:
            ...
            if key in self.game.state.soil and self.game.state.soil[key].get('tilled'):
                ...
                count += 1
        if count:
            self.game.notif(f"Menyiram {count} petak!")
        else:
            self.game.notif("Tidak ada lahan!")
    ...
    def use_plant(self, x, y):
        ...
        if self.game.state.inventory.get(seed_key, 0) <= 0:
            self.game.notif("Benih habis!"); return
        crop = CROPS[self.game.state.seed_key]
        if (self.game.state.get_season() not in crop['seasons']
                and not self.game.state.greenhouse_open):
            self.game.start_dialog(
                [f"{crop['name']} hanya bisa di musim: {', '.join(crop['seasons'])}."],
                "🌱")
            return
    ...
    def use_harvest(self, x, y):
        ...
        if not self.spend_energy(2): return
        crop_id = soil['crop']
        self.game.state.inventory[crop_id] = \
            self.game.state.inventory.get(crop_id, 0) + 1
    ...
    def harvest_wild(self, wild):
        ...
        if kind == 'mandrake':
            ...
            self.game.state.inventory[kind] = \
                self.game.state.inventory.get(kind, 0) + 1
    ...
    def use_axe(self, x, y, tile):
        ...
        self.game.state.inventory['kayu'] = \
            self.game.state.inventory.get('kayu', 0) + 3
    ...
    def give_gift(self):
        ...
        for npc_id, pos in self.game.state.npc_positions.items():
            if pos['scene'] != self.game.state.scene_name:
                continue
            if abs(pos['x'] - px) <= 1.5 and abs(pos['y'] - py) <= 1.5:
                ...
                if not gift_item:
                    self.game.notif(f"{npc_data['name']} tidak menerima hadiah.")
                    return
                ...
                self.game.state.npc_hearts[npc_id] = min(
                    10, self.game.state.npc_hearts[npc_id] + 2)
            ...
            if abs(w['x'] - px) <= 1 and abs(w['y'] - py) <= 1:
                ...
                if kind == 'running_mushroom':
                    ...
                    if random.random() < 0.6:
                        ...
                        sc = SCENES[w['scene']]
                        w['x'] = max(0, min(sc.w - 1, w['x'] + random.randint(-3, 3)))
                        w['y'] = max(0, min(sc.h - 1, w['y'] + random.randint(-3, 3)))
                    ...
                    if random.random() < 0.7:
                        self.game.state.inventory['firefly'] = \
                            self.game.state.inventory.get('firefly', 0) + 1
                        self.game.state.wild_entities.remove(w)
                        self.game.state.captured_supernatural += 1
                        self.game.notif("✨ Tertangkap!")
                        self.game.check_quest_progress()
    ...
    def interact(self):
        ...
        # Cek NPC nearby (radius diperluas untuk naga)
        px, py = self.game.state.get_player_tile()
        for npc_id, pos in self.game.state.npc_positions.items():
            if pos['scene'] != self.game.state.scene_name:
                continue
    ...
    def talk_to_npc(self, npc_id):
        ...
        if not npc_data:
            return
        idx = min(self.game.state.npc_dialog_index.get(npc_id, 0),
                  len(npc_data['talks']) - 1)
        lines = npc_data['talks'][idx]
        self.game.state.npc_dialog_index[npc_id] = min(
            idx + 1, len(npc_data['talks']) - 1)

        if npc_id == 'jin_kebun' and not self.game.state.met_jin:
            self.game.state.met_jin = True
            self.game.check_quest_progress()

        # Animal product (sapi/ayam/kambing → susu/telur/wol)
        if npc_data.get('type') in ['sapi', 'ayam', 'kambing']:
            product = npc_data.get('product')
            if product and not self.game.state.inventory.get(
                    f'{product}_{npc_id}_today', False):
                self.game.state.inventory[product] = \
                    self.game.state.inventory.get(product, 0) + 1
    ...
    def show_npc_schedule(self):
        ...
        for npc_id, pos in self.game.state.npc_positions.items():
            ...
            if abs(pos['x'] - px) <= 2 and abs(pos['y'] - py) <= 2:
                ...
                lines = [f"{npc_data['name']} — Jadwal:",
                         f"Sekarang: {pos['activity']}"]
    ...
    def read_mail(self):
        ...
        self.game.start_dialog([
            ...
            '"— Paman Arsa"',
        ], "📬 Surat", self.game.check_quest_progress)
    ...
    def use_stove(self):
        for c in ['tomat', 'jagung', 'lobak', 'wortel', 'bayam']:
            if self.game.state.inventory.get(c, 0) > 0:
                ...
                if self.game.state.inventory[c] <= 0:
                    del self.game.state.inventory[c]
                self.game.state.energy = min(
                    self.game.state.max_energy, self.game.state.energy + 20)
                self.game.notif("Memasak! +20 energi")
                return
    ...
    def sit_fireplace(self):
        self.game.state.energy = min(
            self.game.state.max_energy, self.game.state.energy + 10)
    ...
    def rest_at_table(self):
        self.game.state.energy = min(
            self.game.state.max_energy, self.game.state.energy + 5)
