...
def generate_characters():
    ...
    # make_naga dari v4 sudah return sprite ukuran besar
    print("   ✓ Generating naga (huge sprite)...")
    for d in ['up', 'down', 'left', 'right']:
        actual_d = 'right' if d == 'left' else d
        for f_idx in range(3):
            actual_f = f_idx if f_idx < 2 else 0  # frame 2 = idle (= frame 0)
            surf = make_naga(actual_d, actual_f)
            # Naga sudah ukuran besar; tidak perlu scale
