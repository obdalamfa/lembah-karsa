...
class PlayerController:
    ...
    def update(self, dt):
        ...
        # 1. Keyboard Input (8-Arah)
        if keys[pygame.K_UP] or keys[pygame.K_w]: dy -= 1
        if keys[pygame.K_DOWN] or keys[pygame.K_s]: dy += 1
        if keys[pygame.K_LEFT] or keys[pygame.K_a]: dx -= 1
        if keys[pygame.K_RIGHT] or keys[pygame.K_d]: dx += 1

        # 2. Mouse Input (Hold-to-Move)
        if mouse_pressed[0]:  
    ...
    def use_plant(self, x, y):
        ...
        if self.game.state.inventory.get(seed_key, 0) <= 0:
            self.game.notif(f"Benih {CROPS[self.game.state.seed_key]['name']} habis!"); return
    ...
    def use_harvest(self, x, y):
        ...
        # Panggil cek quest progress jika metode tersebut ada di app.py 
        # (Kita gunakan try/except agar aman kalau metodenya belum tertulis)
