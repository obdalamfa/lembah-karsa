...
class Game:
    ...
    def handle_mouse_click(self, event):
        """Klik mouse untuk interaksi cepat."""
        if self.mode != 'play':
            return
        mx, my = event.pos
        if my >= VIEW_H * TILE:
            return  # klik di HUD diabaikan

        # Convert to world tile
        world_tx = int((mx + self.cam_x * TILE) / TILE)
        world_ty = int((my + self.cam_y * TILE) / TILE)

        if event.button == 1:  # left click — interaksi
            # Cek NPC di tile itu
            for npc_id, pos in self.state.npc_positions.items():
                ...
                if abs(pos['x'] - world_tx) <= range_x and abs(pos['y'] - world_ty) <= range_y:
                    px, py = self.state.get_player_tile()
                    # Player harus dalam jarak interaksi juga
                    if abs(pos['x'] - px) <= range_x + 1 and abs(pos['y'] - py) <= range_y + 1:
                        self.talk_to_npc(npc_id)
                        return
                    else:
                        self.notif("Terlalu jauh!")
                        return
            # Klik di tile — kalau adjacent ke player, interact
            px, py = self.state.get_player_tile()
            if abs(world_tx - px) <= 1 and abs(world_ty - py) <= 1:
                # Set facing ke target dulu
                if world_tx > px: self.state.facing = 'right'
                elif world_tx < px: self.state.facing = 'left'
                elif world_ty > py: self.state.facing = 'down'
                elif world_ty < py: self.state.facing = 'up'
                self.interact()
        elif event.button == 3:  # right click — pakai alat
            px, py = self.state.get_player_tile()
            if abs(world_tx - px) <= 1 and abs(world_ty - py) <= 1:
                if world_tx > px: self.state.facing = 'right'
                elif world_tx < px: self.state.facing = 'left'
    ...
    def update_smooth_movement(self, dt):
        """Smooth movement 8-arah dengan keyboard ATAU mouse hold."""
        keys = pygame.key.get_pressed()
        self.is_running = keys[pygame.K_LSHIFT] or keys[pygame.K_RSHIFT]

        # === KEYBOARD INPUT (8-arah) ===
        kdx, kdy = 0, 0
        if keys[pygame.K_UP] or keys[pygame.K_w]: kdy -= 1
        if keys[pygame.K_DOWN] or keys[pygame.K_s]: kdy += 1
        if keys[pygame.K_LEFT] or keys[pygame.K_a]: kdx -= 1
        if keys[pygame.K_RIGHT] or keys[pygame.K_d]: kdx += 1
        ...
        if mouse_buttons[0]:  # left mouse held
            ...
            # Hanya kalau di area world (bukan HUD)
            if my < VIEW_H * TILE:
                # Convert screen pos to world pos
                world_x = (mx + self.cam_x * TILE) / TILE
                world_y = (my + self.cam_y * TILE) / TILE
                # Vector dari player ke cursor
                tdx = world_x - self.state.player_x - 0.5  # tengah tile
                tdy = world_y - self.state.player_y - 0.5
                pixel_dist = math.hypot(tdx * TILE, tdy * TILE)
                # Dead zone — kalau cursor terlalu dekat, jangan gerak
                if pixel_dist > 12:  # MOUSE_DEAD_ZONE
                    norm_dist = math.hypot(tdx, tdy)
                    if norm_dist > 0:
                        mdx = tdx / norm_dist
                        mdy = tdy / norm_dist

        # === GABUNG: keyboard prioritas, mouse fallback ===
        if kdx != 0 or kdy != 0:
            dx, dy = float(kdx), float(kdy)
            # Normalize diagonal
            if dx != 0 and dy != 0:
                dx *= 0.707
                dy *= 0.707
        ...
        # === FACING ===
        if abs(dx) > abs(dy):
            self.state.facing = 'right' if dx > 0 else 'left'
        elif abs(dy) > 0.01:
            self.state.facing = 'down' if dy > 0 else 'up'
        ...
        new_x = self.state.player_x + dx * speed_per_ms * dt
        new_y = self.state.player_y + dy * speed_per_ms * dt

        # Cek collision separately untuk X dan Y (slide along walls)
        cur_tx = int(round(self.state.player_x))
        cur_ty = int(round(self.state.player_y))

        # Cek X movement
        if dx != 0:
            test_tx = int(round(new_x))
            if self.can_walk(test_tx, cur_ty):
                self.state.player_x = new_x
        # Cek Y movement
        if dy != 0:
            test_ty = int(round(new_y))
            cur_tx_now = int(round(self.state.player_x))
            if self.can_walk(cur_tx_now, test_ty):
                self.state.player_y = new_y

        # Mark as moving for animation
        self.move_target_x = 1  # truthy untuk indicate moving

        # Check portal at new tile
        new_tx = int(round(self.state.player_x))
        new_ty = int(round(self.state.player_y))
        if (new_tx, new_ty) != (cur_tx, cur_ty):
            scene = SCENES[self.state.scene_name]
            for portal in scene.portals:
                if portal[0] == new_tx and portal[1] == new_ty:
    ...
    def render_world(self):
        ...
        # ═══ LAYER 1: GROUND TILES (selalu paling bawah) ═══
        # Group tiles berdasarkan apakah dia "tall object" (tree, building, fence)
        # Tall objects akan ikut depth-sort dengan karakter; ground tile render dulu
        TALL_TILES = {TR, DT, FN, H, MB}  # objek tinggi yang occlude karakter
        deferred_tall = []  # (y_sort_key, draw_callable)
        ...
        for ty in range(start_ty, end_ty):
            for tx in range(start_tx, end_tx):
                ...
                # Tall objects → defer untuk depth sort
                if t in TALL_TILES:
                    # Render ground rumput dulu sebagai base
                    self.screen.blit(SPRITES['grass'], (px_, py_))
                    # Defer render tall object
                    sort_y = ty * TILE + TILE  # bottom of tile
                    if t == H:
                        # House composite (red roof + beige wall)
                        ...
                        deferred_tall.append((sort_y, lambda p=px_, py=py_: self.screen.blit(SPRITES['tree'], (p, py))))
                    elif t == DT:
                        deferred_tall.append((sort_y, lambda p=px_, py=py_: self.screen.blit(SPRITES['dead_tree'], (p, py))))
                    elif t == FN:
                        deferred_tall.append((sort_y, lambda p=px_, py=py_: self.screen.blit(SPRITES['fence'], (p, py))))
                    elif t == MB:
                        deferred_tall.append((sort_y, lambda p=px_, py=py_: self.screen.blit(SPRITES['mailbox'], (p, py))))
                    continue
        ...
        # ═══ LAYER 2: PARALLAX CLOUD SHADOW (di atas ground, di bawah karakter) ═══
        if not indoor and 'cloud_shadow' in SPRITES:
            cloud_sprite = SPRITES['cloud_shadow']
            cw, ch = cloud_sprite.get_size()
            t = pygame.time.get_ticks()
            # Cloud scroll lambat (parallax effect)
            cloud_offset = int((t * 0.025) % (SCREEN_W + cw))
            for ci in range(3):
                cx = (-cw + ci * (SCREEN_W // 2 + cw // 2) + cloud_offset) % (SCREEN_W + cw) - cw
                cy = 30 + ci * 80
                if 0 < cy < VIEW_H * TILE:
                    self.screen.blit(cloud_sprite, (cx, cy))

        # ═══ LAYER 3: WILD ENTITIES (collect for depth sort) ═══
        is_night = self.state.is_night()
        sprite_list = []  # (sort_y, draw_func)
        ...
        for w in self.state.wild_entities:
            ...
            if -TILE < wpx < SCREEN_W and -TILE < wpy < VIEW_H * TILE:
                ...
                if kind in ('mandrake', 'running_mushroom'):
                    ...
                    sprite_list.append((sort_y, lambda wpx=wpx, wpy=wpy: self.screen.blit(
                        ANIMATED['firefly'][(pygame.time.get_ticks() // 250) % 3], (wpx, wpy))))
                elif kind in SPRITES:
                    sprite_list.append((sort_y, lambda wpx=wpx, wpy=wpy, k=kind: self.screen.blit(SPRITES[k], (wpx, wpy))))

        # ═══ LAYER 4: NPCs (collect for depth sort) ═══
        for npc_id, pos in self.state.npc_positions.items():
            if pos['scene'] != self.state.scene_name: continue
            if pos['x'] < 0: continue
            ...
            if -TILE < npx < SCREEN_W and -TILE < npy < VIEW_H * TILE:
                ...
                def draw_npc(npx_=npx, npy_=npy, ak=anim_key, h=hearts, t=npc_type):
                    # Shadow
                    if 'shadow_small' in SPRITES:
                        self.screen.blit(SPRITES['shadow_small'], (npx_ + 6, npy_ + TILE - 6))
                    if ak in ANIMATED:
                        sprite = ANIMATED[ak]['down'][self.walk_anim_index]
                        self.screen.blit(sprite, (npx_, npy_))
                    if h > 0:
                        for i in range(min(h // 2, 5)):
                            pygame.draw.rect(self.screen, C.ui_red,
                                           (npx_ + 2 + i*5, npy_ - 6, 3, 3))
                sprite_list.append((sort_y, draw_npc))

        # ═══ LAYER 5: PLAYER (collect for depth sort) ═══
