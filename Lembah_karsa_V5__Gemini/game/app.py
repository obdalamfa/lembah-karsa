...
Sudah terintegrasi dengan PlayerController (8-arah/Mouse), Y-Sorting, dan AI Dinamis.
"""
import pygame
import math
import random
...
class Game:
    def __init__(self):
        ...
        pygame.display.set_caption("🌱 Lembah Karsa — Modular Edition")
        ...
        self.running = True
        self.mode = 'play'  # 'play', 'dialog', 'panel', 'transition'
        self.panel = None
        self.panel_select = 0
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if self.mode == 'play':
                    ...
                    elif event.key == pygame.K_f1: self.open_panel('help')
                    ...
                    if event.key == pygame.K_ESCAPE: self.mode = 'play'
                        ...
                        self.handle_panel_action()

    def update(self, dt):
        # Time progression
        if self.mode == 'play':
            self.state.time_minutes += (dt / 1000.0) * INGAME_MINUTES_PER_REAL_SECOND
            if self.state.time_minutes >= 1440:
                self.sleep_next_day()
            
            # Update Player (8-arah & Mouse)
    ...
    def render_world(self):
        ...
        start_tx, end_tx = max(0, int(self.cam_x)), min(scene.w, int(self.cam_x) + VIEW_W + 2)
        start_ty, end_ty = max(0, int(self.cam_y)), min(scene.h, int(self.cam_y) + VIEW_H + 2)
                ...
                if soil and soil.get('tilled'):
                    ...
                    if soil.get('crop'):
                        ...
                        if crop_id in SPRITES['crops']:
                            renderables.append((py_ + TILE, px_, py_, SPRITES['crops'][crop_id][stage]))
                    continue
        ...
        # 5. Parallax
        if not indoor and self.state.weather in ['Cerah', 'Berangin']:
            cloud_off = (pygame.time.get_ticks() * 0.02) % (SCREEN_W + 300)
            self.screen.blit(SPRITES['shadow'], (cloud_off - 300, 100)) # Placeholder cloud shadow
    ...
    def handle_panel_action(self):
        # Delegasi ke panels.py logic
        pass 
    ...
    def sleep_next_day(self):
        ...
        if self.state.day_in_season > DAYS_PER_SEASON:
            self.state.day_in_season = 1
            self.state.season_index = (self.season_index + 1) % 4
            if self.state.season_index == 0: self.state.year += 1
        
        self.state.time_minutes = 360.0 # Bangun jam 6 pagi
        self.state.energy = self.state.max_energy
        # Crop logic
        for k, v in self.state.soil.items():
            if v.get('crop') and v.get('watered'):
                v['age'] += 1
            v['watered'] = False
    ...
    def cycle_seed(self, step):
        seeds = [k for k in CROPS.keys()]
        idx = seeds.index(self.state.seed_key)
        self.state.seed_key = seeds[(idx + step) % len(seeds)]

    def confirm_sleep(self):
        self.start_dialog(["Apakah kamu ingin tidur dan lanjut ke hari berikutnya?"], "Tidur", self.sleep_next_day)
    ...
    def render_panel(self):
        ...
        elif self.panel == 'help': panels.render_panel_help(self, bx, by, bw, bh)
        elif self.panel == 'shop': panels.render_panel_shop(self, bx, by, bw, bh)
        elif self.panel == 'upgrade': panels.render_panel_upgrade(self, bx, by, bw, bh)
