...
from .sprites import (init_sprites, SPRITES, ANIMATED,
                       make_vignette, draw_glow, get_color_grade, draw_char_shadow)
from .sound import init_sound, build_sounds, play as sound_play, get_step_sound
...
class Game:
    def __init__(self):
        ...
        # Sound system (sebelum init_sprites agar mixer siap)
        init_sound()
        ...
        # Vignette overlay (precomputed sekali)
        self.vignette = make_vignette(SCREEN_W, VIEW_H * TILE)

        # Timer untuk efek ambient (ember, glow flicker, dll)
        self._glow_timer = 0
        self._last_tile_x = -1
        self._last_tile_y = -1
    ...
    def update(self, dt):
        ...
        # Particles selalu update (termasuk saat dialog/panel)
        ...
        if self.mode == 'play':
            ...
            # Ambient glow + particle tick
            self._glow_timer += dt
            if self._glow_timer > 120:
                self._glow_timer = 0
                self._tick_ambient_effects()
    ...
    def update_smooth_movement(self, dt):
        ...
        if self.move_target_x is not None:
            ...
            if dist <= move_dist:
                # Sampai di target
                self.state.player_x = float(self.move_target_x)
                self.state.player_y = float(self.move_target_y)
                # Footstep sound + dust particle
                tx_i = int(self.move_target_x)
                ty_i = int(self.move_target_y)
                if (tx_i, ty_i) != (self._last_tile_x, self._last_tile_y):
                    self._last_tile_x, self._last_tile_y = tx_i, ty_i
                    scene = SCENES[self.state.scene_name]
                    if 0 <= tx_i < scene.w and 0 <= ty_i < scene.h:
                        tile_id = scene.tiles[ty_i][tx_i]
                        tname = TILE_NAMES.get(tile_id, 'grass')
                        sound_play(get_step_sound(tname), 0.45)
                        cam_px = -int(self.cam_x * TILE)
                        cam_py = -int(self.cam_y * TILE)
                        if tname in ('dirt', 'tilled_dry', 'grass'):
                            self.particles.spawn_dust(
                                tx_i * TILE + cam_px, ty_i * TILE + cam_py)
                # Cek portal
                self.check_portal_at_target()
                # Lanjut moving kalau masih ada input
                if new_facing and dx == self.move_target_x - cur_tx:
                    next_tx = self.move_target_x + dx
                    next_ty = self.move_target_y + dy
    ...
    def _tick_ambient_effects(self):
        """Partikel + suara ambient tiap ~120ms."""
        scene = SCENES[self.state.scene_name]
        is_night = self.state.is_night()

        # Ambient dust / serbuk sari
        self.particles.tick_ambient(
            self.cam_x, self.cam_y,
            VIEW_W, VIEW_H, 120, is_night
        )

        # Suara api / lentera sesekali
        if self.state.scene_name in ('house', 'village'):
            if random.random() < 0.08:
                sound_play('fire_crackle', 0.18)

        # Ember dari perapian & lenterna yang terlihat
        cam_px = -int(self.cam_x * TILE)
        cam_py = -int(self.cam_y * TILE)
        ...
        for ty in range(start_ty, end_ty):
            for tx in range(start_tx, end_tx):
                t = scene.tiles[ty][tx]
                if t == FP and random.random() < 0.25:
                    wx = tx * TILE + cam_px
                    wy = ty * TILE + cam_py
                    self.particles.spawn_fire_ember(wx, wy)
                elif t == LN and random.random() < 0.15:
                    wx = tx * TILE + cam_px
                    wy = ty * TILE + cam_py
                    self.particles.spawn_fire_ember(wx, wy)

        # Daun dari pohon saat berangin
        if self.state.weather in ('Berangin', 'Badai') and random.random() < 0.3:
            for ty in range(start_ty, end_ty):
                for tx in range(start_tx, end_tx):
                    if scene.tiles[ty][tx] == TR and random.random() < 0.12:
                        wx = tx * TILE + cam_px
                        wy = ty * TILE + cam_py
                        self.particles.spawn_leaf(wx, wy, n=1)

        # Partikel supernatural yang terlihat
        for w in self.state.wild_entities:
            if w['scene'] != self.state.scene_name:
                continue
            if w.get('night_only') and not is_night:
                continue
            wx = w['x'] * TILE + cam_px
            wy = w['y'] * TILE + cam_py
            if -TILE < wx < SCREEN_W and -TILE < wy < VIEW_H * TILE:
                if w['kind'] in ('jin_kebun', 'demit', 'tuyul') and random.random() < 0.15:
                    self.particles.spawn_magic(wx, wy, n=1)
    ...
    def use_hoe(self, x, y, tile):
        ...
        cam_px = -int(self.cam_x * TILE)
        cam_py = -int(self.cam_y * TILE)
        self.particles.spawn_dust(x * TILE + cam_px, y * TILE + cam_py)
    ...
    def use_watering(self, x, y):
        ...
        for cx, cy in cells:
            ...
            if key in self.state.soil and self.state.soil[key].get('tilled'):
                ...
                self.particles.spawn_water_splash(cx * TILE + cam_px, cy * TILE + cam_py)
                count += 1
        if count:
            sound_play('water')
            self.notif(f"Menyiram {count} petak!")
        else:
            self.notif("Tidak ada lahan!")
    ...
    def use_harvest(self, x, y):
        ...
        cam_px = -int(self.cam_x * TILE)
        cam_py = -int(self.cam_y * TILE)
        self.particles.spawn_harvest_burst(x * TILE + cam_px, y * TILE + cam_py)
    ...
    def use_axe(self, x, y, tile):
        ...
        cam_px = -int(self.cam_x * TILE)
        cam_py = -int(self.cam_y * TILE)
        self.particles.spawn_chop_debris(x * TILE + cam_px, y * TILE + cam_py)
        ...
        for npc_id, pos in self.state.npc_positions.items():
            ...
            if abs(pos['x'] - px) <= 1 and abs(pos['y'] - py) <= 1:
                ...
                self.particles.spawn_sparkle(
                    pos['x'] * TILE + cam_px, pos['y'] * TILE + cam_py, n=5)
            ...
            if abs(w['x'] - px) <= 1 and abs(w['y'] - py) <= 1:
                ...
                if kind == 'running_mushroom':
                    ...
                    if random.random() < 0.6:
                        ...
                        sound_play('capture')
                        self.particles.spawn_capture_burst(
                            w['x'] * TILE + cam_px, w['y'] * TILE + cam_py)
    ...
    def check_quest_progress(self):
        ...
        st = s.quest_stage
        advanced = False
        if st == 0 and s.mail_read: s.quest_stage = 1; advanced = True
        elif st == 1 and s.stats['lobak_planted'] >= 3 and s.stats['watered'] >= 3:
            s.quest_stage = 2; advanced = True
        elif st == 2 and s.stats['lobak_harvested'] >= 3:
            s.quest_stage = 3; s.shop_unlocked = True; advanced = True
            self.notif("🎉 Warung terbuka!")
        elif st == 3 and s.stats['earned'] >= 150: s.quest_stage = 4; advanced = True
        elif st == 4 and (s.upgrades['hoe'] or s.upgrades['water']):
            s.quest_stage = 5; advanced = True
        elif st == 5 and s.stats['corn_harvested'] >= 2: s.quest_stage = 6; advanced = True
        elif st == 6 and s.stats['gifts'] >= 3: s.quest_stage = 7; advanced = True
        elif st == 7 and s.captured_supernatural >= 1: s.quest_stage = 8; advanced = True
        elif st == 8 and s.met_jin: s.quest_stage = 9; advanced = True
        elif st == 9 and s.day >= DAYS_PER_SEASON * 4:
            s.quest_stage = 10; advanced = True
            self.start_dialog([
                "🌻 SATU TAHUN BERLALU!",
    ...
    def advance_dialog(self):
        ...
        if self.typewriter_progress < len(current):
            ...
            sound_play('dialog', 0.5)
            return
        sound_play('dialog', 0.7)
        self.dialog_index += 1
    ...
    def render_world(self):
        ...
        # Glow — sumber cahaya (api, lentera, supernatural)
        ...
        for ty in range(start_ty, end_ty):
            for tx in range(start_tx, end_tx):
                ...
                py_ = ty * TILE + cam_py
                if t == FP:
                    flicker = 0.85 + 0.15 * math.sin(pygame.time.get_ticks() * 0.007)
                    draw_glow(self.screen, px_ + TILE//2, py_ + TILE//2,
                              (255, 160, 40), 64, intensity=flicker * (1.4 if is_night else 0.7))
                elif t == LN:
                    flicker = 0.9 + 0.1 * math.sin(pygame.time.get_ticks() * 0.005)
                    draw_glow(self.screen, px_ + TILE//2, py_ + TILE//4,
                              (255, 215, 80), 48, intensity=flicker * (1.2 if is_night else 0.5))

        # Glow supernatural entities
        for w in self.state.wild_entities:
            if w['scene'] != self.state.scene_name: continue
            if w.get('night_only', False) and not is_night: continue
            wpx = w['x'] * TILE + cam_px
            wpy = w['y'] * TILE + cam_py
            if -TILE*2 < wpx < SCREEN_W + TILE*2 and -TILE*2 < wpy < VIEW_H * TILE + TILE*2:
                kind = w['kind']
                if kind == 'firefly':
                    flicker = 0.7 + 0.3 * math.sin(pygame.time.get_ticks() * 0.012)
                    draw_glow(self.screen, wpx + TILE//2, wpy + TILE//2,
                              (180, 255, 140), 36, intensity=flicker)
                elif kind == 'jin_kebun':
                    pulse = 0.6 + 0.4 * math.sin(pygame.time.get_ticks() * 0.004)
                    draw_glow(self.screen, wpx + TILE//2, wpy + TILE//2,
                              (180, 100, 255), 52, intensity=pulse)
                elif kind in ('demit', 'tuyul'):
                    draw_glow(self.screen, wpx + TILE//2, wpy + TILE//2,
                              (120, 40, 200), 32, intensity=0.5)

        # Bayangan karakter (shadow elips)
        for npc_id, pos in self.state.npc_positions.items():
            if pos['scene'] != self.state.scene_name or pos['x'] < 0: continue
            nx = pos['x'] * TILE + cam_px
            ny = pos['y'] * TILE + cam_py
            if -TILE < nx < SCREEN_W and -TILE < ny < VIEW_H * TILE:
                draw_char_shadow(self.screen, int(nx), int(ny), TILE)
        ppx = self.state.player_x * TILE + cam_px
        ppy = self.state.player_y * TILE + cam_py
        draw_char_shadow(self.screen, int(ppx), int(ppy), TILE)

        # Partikel (dunia)
        self.particles.render(self.screen, cam_px, cam_py, VIEW_H * TILE)

        # Weather — hujan lebih hidup
        if self.state.weather in ('Hujan', 'Badai') and not indoor:
            t_ms = pygame.time.get_ticks()
            n_drops = 80 if self.state.weather == 'Badai' else 50
            for i in range(n_drops):
                rx = (i * 41 + int(t_ms * 0.35)) % SCREEN_W
                ry = (i * 23 + int(t_ms * 0.5)) % (VIEW_H * TILE)
                pygame.draw.line(self.screen, (130, 185, 255, 180),
                                 (rx, ry), (rx - 2, ry + 8))
            # Splash partikel sesekali
            if random.random() < 0.15:
                self.particles.spawn_rain_splash(
                    random.randint(0, SCREEN_W) - cam_px,
                    random.randint(0, VIEW_H * TILE) - cam_py)

        # Berangin — gerak dedaunan
        if self.state.weather in ('Berangin', 'Badai') and not indoor:
            if random.random() < 0.04:
                tx_r = random.randint(start_tx, max(start_tx, end_tx - 1))
                ty_r = random.randint(start_ty, max(start_ty, end_ty - 1))
                if 0 <= ty_r < scene.h and 0 <= tx_r < scene.w:
                    if scene.tiles[ty_r][tx_r] == TR:
                        self.particles.spawn_leaf(
                            tx_r * TILE + cam_px, ty_r * TILE + cam_py, n=2)

        # Night overlay — deep & atmospheric
        if is_night and not indoor:
            darkness = 100 if hour < 5 or hour >= 23 else 65
            dark = pygame.Surface((SCREEN_W, VIEW_H * TILE))
            dark.set_alpha(darkness)
            dark.fill((5, 8, 38))
            self.screen.blit(dark, (0, 0))

        # Color grading overlay (Octopath tone shift per waktu hari)
        cg = get_color_grade(hour, self.state.weather in ('Hujan', 'Badai'))
        if cg[3] > 5 and not indoor:
            grade_surf = pygame.Surface((SCREEN_W, VIEW_H * TILE))
            grade_surf.fill((cg[0], cg[1], cg[2]))
            grade_surf.set_alpha(cg[3])
            self.screen.blit(grade_surf, (0, 0))

        # Vignette — tepi gelap Octopath
