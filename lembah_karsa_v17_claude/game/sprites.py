...
def load_char_anim(char_name, fallback_color, huge=False):
    """Load animasi karakter 4 arah × 3 frame.
    Naga huge=True → render 96×64 (3×2 tile)."""
    anim = {}
    base_path = f"assets/chars/{char_name}"
    target_w = 96 if huge else TILE
    target_h = 64 if huge else int(TILE * 1.5)
...
def init_sprites():
    ...
    ANIMATED['npc_naga'] = load_char_anim('naga', (200, 80, 60), huge=True)
