...
# ─── 3 OUTDOOR DASAR (V15) ───────────────────────────
def build_farm():
    """Kebun Paman Arsa — farmable plot + rumah + kandang ternak."""
    W_, H_ = 25, 20
...
def build_mountain():
    """Lereng Gunung — hutan + entrance ke gua naga + ke kuburan."""
    W_, H_ = 30, 25
    m = [[G]*W_ for _ in range(H_)]
    ...
    m[3][14] = DR; m[3][15] = DR  # pintu ke naga_cave
    # Hutan
    for y in range(4, H_):
        for x in range(W_):
            if x < 12 or x > 18:
    ...
    # Beberapa kuburan tua di bawah-kiri (akses ke cemetery scene via path)
