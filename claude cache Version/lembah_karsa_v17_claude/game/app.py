...
from .config import (
    ...
    TILE_NAMES, TOOLS, G, D, P, W, FL, STR_T, BD
)
...
class Game:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if self.mode == 'play':
                    ...
                    elif event.key == pygame.K_z: self.player_ctrl.attack()
                    elif event.key == pygame.K_x: self.player_ctrl.use_pickaxe()
    ...
    def update(self, dt):
        if self.mode == 'play':
            ...
            self.player_ctrl.update_combat(dt)
    ...
    def render_world(self):
        ...
        for npc_id, pos in self.state.npc_positions.items():
            if pos['scene'] == self.state.scene_name and pos['x'] >= 0:
                ...
                if anim_key in ANIMATED:
                    ...
                    sprite = ANIMATED[anim_key][facing][self.walk_anim_index % 3]
                    sh = sprite.get_height()
                    # Offset: kaki menapak bottom tile
                    off_y = TILE - sh
                    renderables.append((py_ + TILE - 1, px_, py_, SPRITES['shadow']))
                    renderables.append((py_ + TILE, px_, py_ + off_y, sprite))

        # Render mob (di dungeon)
        for mob in self.state.mobs:
            mpx = mob['x'] * TILE + cam_px
            mpy = mob['y'] * TILE + cam_py
            mob_sprite = pygame.Surface((TILE, TILE), pygame.SRCALPHA)
            flash = mob.get('dmg_flash_ms', 0) > 0
            color = (255, 200, 200) if flash else (180, 60, 80)
            if mob['kind'] == 'naga_boss':
                color = (220, 80, 60); size = TILE * 2
                mob_sprite = pygame.Surface((size, size), pygame.SRCALPHA)
                pygame.draw.circle(mob_sprite, color, (size//2, size//2), size//2-2)
                renderables.append((mpy + size, mpx - TILE//2, mpy - TILE//2, mob_sprite))
            else:
                pygame.draw.rect(mob_sprite, color, (4, 4, TILE-8, TILE-8))
                pygame.draw.rect(mob_sprite, (40,20,20), (4, 4, TILE-8, TILE-8), 2)
                # HP bar mini di atas mob
                hp_pct = max(0, mob['hp'] / max(1, mob.get('max_hp', mob['hp'])))
                pygame.draw.rect(mob_sprite, (60,20,20), (2, 0, TILE-4, 3))
                pygame.draw.rect(mob_sprite, (220,60,80), (2, 0, int((TILE-4)*hp_pct), 3))
                renderables.append((mpy + TILE, mpx, mpy, mob_sprite))
        ...
        renderables.append((ppy + TILE, ppx, ppy + off_y, sprite))
    ...
    def render_hud(self):
        ...
        panel = pygame.Surface((SCREEN_W, hud_h), pygame.SRCALPHA)
        pygame.draw.rect(panel, (20, 15, 30, 220), (0, 0, SCREEN_W, hud_h)) 
        pygame.draw.line(panel, (180, 160, 120, 255), (0, 0), (SCREEN_W, 0), 3) 
        self.screen.blit(panel, (0, hud_y))
        
        self.draw_text("ENERGI", 30, hud_y + 15, (200, 200, 200))
        
        bar_w = 200
        bar_h = 18
        fill_w = int((self.state.energy / self.state.max_energy) * bar_w)
        fill_w = max(0, min(fill_w, bar_w)) 
        e_color = (100, 220, 100) if self.state.energy > 30 else (255, 80, 80)
        
        pygame.draw.rect(self.screen, (50, 50, 50), (100, hud_y + 15, bar_w, bar_h), border_radius=5)
        if fill_w > 0:
            pygame.draw.rect(self.screen, e_color, (100, hud_y + 15, fill_w, bar_h), border_radius=5)
        pygame.draw.rect(self.screen, (200, 200, 200), (100, hud_y + 15, bar_w, bar_h), 2, border_radius=5)
        
        e_text = self.font.render(f"{self.state.energy}/{self.state.max_energy}", True, (255, 255, 255))
        self.screen.blit(e_text, (100 + bar_w//2 - e_text.get_width()//2, hud_y + 14))

        # HP bar (combat)
        self.draw_text("HP", 30, hud_y + 38, (255, 200, 200))
        hp_fill = max(0, int((self.state.hp / max(1, self.state.max_hp)) * bar_w))
        pygame.draw.rect(self.screen, (50, 30, 30), (100, hud_y + 38, bar_w, bar_h), border_radius=5)
        if hp_fill > 0:
            hp_color = (220, 60, 60) if self.state.hp < 30 else (220, 100, 100)
            pygame.draw.rect(self.screen, hp_color, (100, hud_y + 38, hp_fill, bar_h), border_radius=5)
        pygame.draw.rect(self.screen, (200, 200, 200), (100, hud_y + 38, bar_w, bar_h), 2, border_radius=5)
        hp_text = self.font.render(f"{self.state.hp}/{self.state.max_hp}", True, (255,255,255))
        self.screen.blit(hp_text, (100 + bar_w//2 - hp_text.get_width()//2, hud_y + 37))

        # Dungeon level indicator
        if self.state.dungeon_level > 0:
            self.draw_text(f"⛏ Lv.{self.state.dungeon_level}", 320, hud_y + 38, (200, 220, 255))
        
        self.draw_text(f"💰 {self.state.gold} G", 30, hud_y + 55, (255, 215, 0))

        tool = TOOLS[self.state.tool_index]
        pygame.draw.rect(self.screen, (40, 30, 50), (350, hud_y + 20, 200, 60), border_radius=10)
        pygame.draw.rect(self.screen, (100, 80, 120), (350, hud_y + 20, 200, 60), 2, border_radius=10)
        self.draw_text(f"🛠️ {tool}", 370, hud_y + 30, (150, 220, 255))
        
        if tool == 'Tanam':
            seed = CROPS[self.state.seed_key]['name']
            qty = self.state.inventory.get(self.state.seed_key + '_seed', 0)
            self.draw_text(f"🌱 {seed} ({qty})", 370, hud_y + 55, (200, 255, 200))

        box_x = SCREEN_W - 250
        pygame.draw.rect(self.screen, (30, 20, 40), (box_x, hud_y + 15, 220, 70), border_radius=8)
        self.draw_text(f"📅 Thn {self.state.year} | {self.state.get_season_name()}", box_x + 15, hud_y + 25, (220, 200, 180))
        self.draw_text(f"⏰ {self.state.get_time_str()}  ({self.state.weather})", box_x + 15, hud_y + 55, (255, 255, 255))

    def draw_text(self, text, x, y, color=(220,200,255)):
    ...
    def descend_dungeon(self):
        """Turun ke level dungeon berikutnya. Generate ulang."""
