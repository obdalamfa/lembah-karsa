...
def build_farm():
    ...
    for y in range(2, 8):
        for x in range(15, 22):
            m[y][x] = STR
            if y == 2 or y == 7 or x == 15 or x == 21: m[y][x] = PEN
    m[7][18] = GT
    for x in range(0, W_): m[0][x] = FN; m[H_-1][x] = FN
    for y in range(0, H_):
        m[y][0] = FN
...
def build_mountain():
    ...
    for y in range(0, 4):
        for x in range(0, W_): m[y][x] = CV_W
    m[3][14] = DR; m[3][15] = DR 
    for y in range(4, H_):
        for x in range(0, W_):
            if x < 12 or x > 18:
...
def _build_indoor_room(name, display, furniture_layout, npc_outdoor_pos):
    ...
    return Scene(name, display, m, portals=[(7, 11, npc_outdoor_pos[0], npc_outdoor_pos[1], npc_outdoor_pos[2])], indoor=True)

def build_house():
    return _build_indoor_room('house', 'Rumah Kamu', [(1, 1, BD), (2, 1, BD), (1, 2, CH), (4, 1, BS), (5, 1, BS), (1, 3, MR), (1, 4, PP), (7, 1, FP), (9, 1, CL), (6, 3, TB), (7, 3, TB), (11, 1, ST), (12, 1, ST), (11, 2, SH), (12, 2, SH), (13, 5, PP)], ('farm', 3, 5))
def build_shop():
    return _build_indoor_room('shop', 'Warung Bu Sari', [(x, 4, CT) for x in range(1, 14)] + [(x, 1, SH) for x in range(1, 14)], ('town', 4, 9))
def build_clinic():
    return _build_indoor_room('clinic', 'Klinik Pak Raka', [(1, 1, BD), (2, 1, BD), (12, 1, BD), (13, 1, BD), (1, 3, TB), (13, 3, BS)], ('town', 11, 9))
def build_studio():
    return _build_indoor_room('studio', 'Studio Maya', [(6, 1, TB), (7, 1, TB), (8, 1, TB), (1, 3, BS), (2, 3, BS)], ('town', 22, 9))
def build_smith():
    return _build_indoor_room('smith', 'Bengkel Budi', [(6, 1, TB), (1, 2, SH), (12, 1, FP)], ('town', 7, 22))
def build_greenhouse():
    ...
    return Scene('greenhouse', 'Rumah Kaca', m, portals=[(7, 11, 'town', 21, 22)], indoor=True)
def build_naga_cave():
    W_, H_ = 15, 12
    ...
    m[H_-1][7] = DR
    for x, y in [(2, 2), (12, 2), (2, 9), (12, 9), (3, 4), (11, 6)]: m[y][x] = CV_W
    # STAIRS_DOWN ke dungeon level 1 (di tengah)
    m[5][7] = STAIRS_DOWN
    return Scene('naga_cave', 'Gua Sang Hyang', m, portals=[(7, 11, 'mountain', 14, 4)], indoor=True)
...
SCENES = {
    'farm': build_farm(), 'town': build_town(), 'mountain': build_mountain(),
    'house': build_house(), 'shop': build_shop(), 'clinic': build_clinic(),
    'studio': build_studio(), 'smith': build_smith(), 'greenhouse': build_greenhouse(),
