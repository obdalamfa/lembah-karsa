...
class PlayerController:
    ...
    def update(self, dt):
        ...
        # State machine: lock movement saat attacking
        if self.game.state.attack_cooldown_ms > 200:  # window 200ms = swing animasi
            self.move_target_x = None
            return
    ...
    def check_portal_at_target(self):
        ...
        # Cek tile dungeon: STAIRS_DOWN/UP
        from .config import STAIRS_DOWN, STAIRS_UP
        st = self.game.state
        # Get current tile
        cur_tile = None
        if st.dungeon_level > 0 and st.dungeon_tiles:
            if 0 <= ty < len(st.dungeon_tiles) and 0 <= tx < len(st.dungeon_tiles[0]):
                cur_tile = st.dungeon_tiles[ty][tx]
        else:
            if 0 <= ty < scene.h and 0 <= tx < scene.w:
                cur_tile = scene.tiles[ty][tx]
        if cur_tile == STAIRS_DOWN:
            self.game.descend_dungeon()
            return
        if cur_tile == STAIRS_UP:
            self.game.ascend_dungeon()
            return
    ...
    def talk_to_npc(self, npc_id):
        ...
        if npc_id == 'sari' and self.game.state.shop_unlocked:
            ...
            callback = lambda: self.game.open_panel('craft')
    ...
    # ═══════════════════════════════════════════════
    # COMBAT (action: tekan Z untuk swing)
    # ═══════════════════════════════════════════════
    def attack(self):
        """Swing alat/pedang ke arah facing. Hit semua mob dalam radius."""
        from .config import (PLAYER_ATTACK_RANGE, PLAYER_ATTACK_COOLDOWN_MS,
                             TOOL_DAMAGE)
        st = self.game.state
        if not hasattr(st, 'attack_cooldown_ms'): st.attack_cooldown_ms = 0
        if st.attack_cooldown_ms > 0: return
        # Damage berdasarkan sword
        ...
        if hit:
            self.game.notif(f"⚔ Hit {hit}!")

    def use_pickaxe(self):
        """Tambang ore tile di depan. Drop mineral sesuai jenis."""
        ...
        if st.dungeon_level == 0:
            self.game.notif("Pickaxe hanya di gua!"); return
        if st.pickaxe_tier <= 0:
            self.game.notif("Belum punya pickaxe!"); return
        tiles = st.dungeon_tiles
        if not (0 <= fy < len(tiles) and 0 <= fx < len(tiles[0])): return
        t = tiles[fy][fx]
        # Mining: ore drop sesuai jenis
        ore_map = {ORE_TBG:('tembaga',1), ORE_BSI:('besi',1),
                   ORE_EMS:('emas',1), ORE_KRS:('kristal',1),
                   ORE_MTH:('mithril',1), CRYS:('kristal',2), CV_W:None}
        # Pickaxe tier requirement
        tier_req = {ORE_TBG:1, ORE_BSI:2, ORE_EMS:3, ORE_KRS:3,
                    ORE_MTH:4, CRYS:4, CV_W:1}
        if t not in tier_req:
            self.game.notif("Tidak bisa ditambang."); return
        if st.pickaxe_tier < tier_req[t]:
            self.game.notif(f"Pickaxe terlalu lemah (butuh tier {tier_req[t]})!")
            return
        if not self.spend_energy(2): return
        tiles[fy][fx] = MINED
        if t in ore_map and ore_map[t]:
            mat, qty = ore_map[t]
            st.inventory[mat] = st.inventory.get(mat, 0) + qty
            st.stats['minerals_mined'] = st.stats.get('minerals_mined', 0) + qty
            self.game.notif(f"⛏ +{qty} {mat}")
        # Cangkul tile dirt: 30% chance staircase
        elif t == D:
            import random
            if random.random() < 0.3:
                tiles[fy][fx] = STAIRS_DOWN
                self.game.notif("✨ Tangga muncul!")
    ...
    def update_combat(self, dt):
        ...
        # Mob AI: kejar player + serang jika dekat
        import math
        ...
        for mob in list(st.mobs):
            ...
            if d > 0.8:
                ...
                # Attack player
                if 'attack_cooldown_ms' not in mob: mob['attack_cooldown_ms'] = 0
                ...
                if mob['attack_cooldown_ms'] <= 0 and st.invuln_timer_ms <= 0:
                    ...
                    if st.hp <= 0:
                        # Player KO: respawn di rumah, HP penuh
                        st.hp = st.max_hp
                        st.scene_name = 'house'
                        st.player_x, st.player_y = 7, 9
                        st.dungeon_level = 0
                        st.mobs = []
                        st.energy = max(10, st.energy // 2)
                        self.game.notif("💀 Pingsan! Bangun di rumah.")
